...
public class HerpJudge {

	// *************************
	static Board g = Judge.gameBoard;
	...
	public static Board strat(char[][] map, int[][] goldMap, int[][] stoneMap,
		...
		Judge.gameBoard = new Board(fullMap, available, mode);
		return Judge.gameBoard;
	}
	...
	public static String action(String id, String command) {
		...
		if (isWorker(id)) {
			...
			if (splitedCommand[0].equals("move")) {
				...
				return Judge.gameBoard.workerMoveHerp(id, Row, Col);

			} // mind checked ;)
				// wwwwwwwwwwwwwwwwwwww
				// workerWorkAt
			else if (splitedCommand[0].equals("work")) {
				return Judge.gameBoard.workerWorkAtH(id, splitedCommand[1]);
			}
			// wwwwwwwwwwwwwwwwwwww
			// workerBuild
				...
				return Judge.gameBoard.workerBuildH(id, Type, Row, Col);
			} else
			...
			if (command.equals("sell"))
				return Judge.gameBoard.sellH(id);

			if (id.split("_")[1].equals("1")) {
				if (command.equals("train"))
					return Judge.gameBoard.trainWorkerInH(id);
				else
					return "invalid command";
			}
			/*
			 ...
			 */
				...
				if (command.equals("train"))
					return Judge.gameBoard.trainBoatInH(id);
				else
					return "invalid command";
			}
			/*
			 ...
			 */
				...
				if (command.equals("train"))
					return Judge.gameBoard.trainScholarInH(id);
				else
					return "invalid command";
			}
			/*
			 ...
			 */
				...
				return Judge.gameBoard.exchangeH(amount, from, to, id);
			} else if (id.split("_")[1].equals("10")) {
				if (command.equals("axer"))
					return Judge.gameBoard.trainAxerInH(id);
				// TODO
				else if (command.equals("darter"))
					return Judge.gameBoard.trainDarterInH(id);
				else
					return "invalid command";
			} else if (id.split("_")[1].equals("11")) {
				if (command.equals("nigga"))
					return Judge.gameBoard.trainNiggaInH(id);
				else if (command.equals("cluber"))
					return Judge.gameBoard.trainCluberInH(id);
				// TODO
				else
					return "invalid command";
			}
			...
			if (splitedCommand[0].equals("research")) {
				...
				return Judge.gameBoard.doResearchH(id, research);
			} else
				return "invalid command";
			...
			if (splitedCommand[0].equals("move")) {
				...
				return Judge.gameBoard.soldierMoveH(id, Row, Col);
			} else if (splitedCommand[0].equals("attack")) {
				Judge.gameBoard.soldierAttackH(id,splitedCommand[1]);
			}
			...
			if (splitedCommand[0].equals("set")) {
				...
				Judge.gameBoard.cheatSetResource(resource, amount);

			} else if (splitedCommand[0].equals("worker")) {
				int row, col;
				row = Integer.parseInt(splitedCommand[1]);
				col = Integer.parseInt(splitedCommand[2]);
				return Judge.gameBoard.cheatMakeWorkerAt(row - 1, col - 1);
				...
				row = Integer.parseInt(splitedCommand[2]);
				col = Integer.parseInt(splitedCommand[3]);
				Judge.gameBoard.cheatMoveWorkerAt(workerId, row - 1, col - 1);
				...
				row = Integer.parseInt(splitedCommand[2]);
				col = Integer.parseInt(splitedCommand[3]);
				return Judge.gameBoard.cheatMakeBuildingAt(buildingType, row - 1,
						col - 1);

			} else if (splitedCommand[0].equals("scholar")) {
				return Judge.gameBoard.cheatMakeScholar();

			} else if (splitedCommand[0].equals("research")) {
				int research = Integer.parseInt(splitedCommand[1]);
				Judge.gameBoard.cheatResearch(research);

			} else if (splitedCommand[0].equals("explore")) {
				if (splitedCommand[1].equals("all"))
					Judge.gameBoard.cheatExploreAll();
				else {
					int row, col;
					row = Integer.parseInt(splitedCommand[1]);
					col = Integer.parseInt(splitedCommand[2]);
					Judge.gameBoard.cheatExplore(row - 1, col - 1);
				}
			}
	...
	public static String[] nextTurn() {
		return Judge.gameBoard.nextTurn();
	}
